...
Poly karatsuba(const Poly& p1, const Poly& p2) {
    int n = p1.size(), m = p2.size();
    if (n <= 32 || m <= 32) return multiply_cpu(p1, p2);

    int mid = std::max(n, m) / 2;

    Poly low1(p1.begin(), p1.begin() + std::min((int)p1.size(), mid));
    Poly high1;
    if ((int)p1.size() > mid) high1 = Poly(p1.begin() + mid, p1.end());

    Poly low2(p2.begin(), p2.begin() + std::min((int)p2.size(), mid));
    Poly high2;
    if ((int)p2.size() > mid) high2 = Poly(p2.begin() + mid, p2.end());

    Poly z0 = karatsuba(low1, low2);
    Poly z2;
    if (!high1.empty() && !high2.empty()) z2 = karatsuba(high1, high2);
    ...
    for (size_t i = 0; i < z0.size() && i < z1.size(); i++) z1[i] -= z0[i];
    for (size_t i = 0; i < z2.size() && i < z1.size(); i++) z1[i] -= z2[i];

    Poly result(n + m - 1, 0);
    for (size_t i = 0; i < z0.size(); i++) result[i] += z0[i];
    for (size_t i = 0; i < z1.size() && i + mid < result.size(); i++) result[i + mid] += z1[i];
    for (size_t i = 0; i < z2.size() && i + 2*mid < result.size(); i++) result[i + 2*mid] += z2[i];
...
public:
    ...
    MetalCompute() {
        ...
        id<MTLFunction> regular_fn = [library newFunctionWithName:@"poly_multiply_regular"];
        id<MTLFunction> tiled_fn = [library newFunctionWithName:@"poly_multiply_tiled"];

        regular_pipeline = [device newComputePipelineStateWithFunction:regular_fn error:&error];
        tiled_pipeline = [device newComputePipelineStateWithFunction:tiled_fn error:&error];
    }
    ...
    Poly multiply_regular(const Poly& p1, const Poly& p2) {
        ...
        id<MTLBuffer> buf_p1 = [device newBufferWithBytes:p1.data() length:n*sizeof(int) options:MTLResourceStorageModeShared];
        id<MTLBuffer> buf_p2 = [device newBufferWithBytes:p2.data() length:m*sizeof(int) options:MTLResourceStorageModeShared];
        id<MTLBuffer> buf_result = [device newBufferWithLength:result_size*sizeof(int) options:MTLResourceStorageModeShared];
        ...
        NSUInteger threads_per_group = regular_pipeline.maxTotalThreadsPerThreadgroup;
        MTLSize grid = MTLSizeMake(result_size, 1, 1);
        MTLSize group = MTLSizeMake(std::min((NSUInteger)result_size, threads_per_group), 1, 1);
...
int main(int argc, char** argv) {
    ...
    Poly p1 = random_poly(size);
    Poly p2 = random_poly(size);
    ...
    double metal_regular_time = bench([&] { metal_regular_result = metal.multiply_regular(p1, p2); });
    double metal_tiled_time = bench([&] { metal_tiled_result = metal.multiply_tiled(p1, p2); });
    ...
    printf("%-25s %10.2f ms  %6.2fx\n", "cpu karatsuba", karatsuba_time, cpu_time / karatsuba_time);
    printf("%-25s %10.2f ms  %6.2fx\n", "metal regular", metal_regular_time, cpu_time / metal_regular_time);
    printf("%-25s %10.2f ms  %6.2fx\n", "metal tiled", metal_tiled_time, cpu_time / metal_tiled_time);
