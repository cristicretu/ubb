...
void testCreate() {
  ...
  SetIterator it = s.iterator();  // iterator on the empty set should be invalid
                                  // from the start
...
void testRemove() {
  ...
  for (int i = -100; i < 100;
       i++) {  // delete everything (including inexistent elements)
    if (i % 2 == 0) {
  ...
  // printM(m);
  for (int i = 100; i > -100; i--) {  // delete in descending order (reverse
                                      // order compared to the adding)
    if (i % 2 == 0) {
      assert(m.remove(i) == true);
    } else {
      assert(m.remove(i) == false);
...
// we don't know how the set is represented and in which order the elements are
// stored or printed, we can only test general thing
