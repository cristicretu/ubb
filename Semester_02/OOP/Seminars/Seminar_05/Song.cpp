...
Song::Song(const std::string& artist, const std::string& title,
           const Duration& duration, const std::string& source) {
  this->artist = artist;
  this->title = title;
...
void Song::play() {
  ...
  //               SW_SHOWMAXIMIZED);
  std::cout << "Playing song " << this->getTitle() << " by "
            << this->getArtist() << " from " << this->getSource() << "\n";
}
...
istream& operator>>(istream& is, Song& s) {
  ...
  if (durationTokens.size() != 2) return is;
...
ostream& operator<<(ostream& os, const Song& s) {
  os << s.artist << "," << s.title << "," << s.duration.getMinutes() << ":"
     << s.duration.getSeconds() << "," << s.source << "\n";
  return os;
}
