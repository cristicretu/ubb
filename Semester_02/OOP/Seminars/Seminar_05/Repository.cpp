...
void Repository::removeSong(const Song& s) {
  ...
  if (it == this->songs.end()) throw InexistenSongException{};
  this->songs.erase(it);
  this->writeToFile();
}

Song Repository::findByArtistAndTitle(const std::string& artist,
                                      const std::string& title) const {
  for (auto s : this->songs) {
    if (s.getArtist() == artist && s.getTitle() == title) return s;
  }
...
void Repository::readFromFile() {
  ...
  if (!file.is_open()) throw FileException("The file could not be opened!");

  Song s;
  while (file >> s) this->songs.push_back(s);
