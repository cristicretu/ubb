...
class DogTypeRepository {
 ...
 public:
  ...
  DynArray<std::string> getBreeds() const { return this->breeds; }

  int getNumberOfBreeds() const { return this->breeds.getSize(); }
  std::string getBreed(int index) const { return this->breeds[index]; }

  int findBreed(const std::string &breed) const {
    for (int i = 0; i < this->breeds.getSize(); i++) {
  ...
  std::string getRandomBreed() const {
    int index = rand() % this->breeds.getSize();
    return this->breeds[index];
  }

  std::string getRandomName() const {
    int index = rand() % this->names.getSize();
    return this->names[index];
  }

  // DynArray<std::string> getNames() const;
};
