...
exec UpdateDatabaseVersion 4;

select * from Results


insert into Results values (50, 1, 52, 5320, 25, 'iwjdjaoiwjidjawoidajwoidjoaiw' , 3, 1, 1);
...
create view DriversView as
    select driverId, firstName, lastName, country
from Drivers;

select * from DriversView;

create view ResultsView as
    select
    r.resultId, r.points, r.finishStatus, r.raceId, r.driverId, r.teamId,
    d.firstName, d.lastName, t.teamName
from Results r
join Drivers d on r.driverId = d.driverId
join Teams t on r.teamId = t.teamId;
...
create view TeamPoints as
    select t.teamId, t.teamName, sum(r.points) as totalPoints
from Results r
join Teams t on r.teamId = t.teamId
group by t.teamId, t.teamName;

select * from TeamPoints order by totalPoints desc;



create procedure AddSponsors
    @count int
...
begin
    ...
    while @i < @count
    begin
        insert into Sponsors (sponsorName) values ('sponsor' + cast(@i * rand() as varchar));
        set @i = @i + 1;
    end;
end;
...
WHERE type_desc = 'FOREIGN_KEY_CONSTRAINT'
    AND OBJECT_NAME(parent_object_id) = 'TeamEngines';


create procedure AddEngines
    @count int
...
begin
    ...
    while @i < @count
    begin
        insert into Engines (manufacturer, model, power) values ('manufacturer' + cast(@i * rand() as varchar), 'model' + cast(@i * rand() as varchar), 1000 + @i * rand() * 100);
        set @i = @i + 1;
    end;
end;
...
begin
   declare @i int = 0;
    ...
    begin
        ...
        select top 1 @driverId = driverId from Drivers order by newid();
        declare @raceId int;
        select top 1 @raceId = raceId from Races order by newid();
        insert into Penalties (penaltyType, penaltyDescription, penaltyDroppedPositions, driverId, raceId) values ('type' + cast(@i * rand() * 100 as varchar), 'desc' + cast(@i * rand() * 100 as varchar),
                                                                                                                             @i,@driverId, @raceId);
        set @i = @i + 1;
    end;
end;
